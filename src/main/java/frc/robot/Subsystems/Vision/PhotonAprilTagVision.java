...
import frc.Util.BreadUtil.GeomUtil;
import frc.Util.BreadUtil.SingleTagAdjusters;
import frc.Util.BreadUtil.TimestampedVisionUpdate;
...
public class PhotonAprilTagVision extends SubsystemBase {
  ...
  public PhotonAprilTagVision(PhotonSim... cameras) {
  ...
  public void periodic() {
    ...
    for (int instanceIndex = 0; instanceIndex < cameras.length; instanceIndex++) {
      ...
      if (shouldUseMultiTag) {
        ...
        Pose3d tagPos = VisionConstants.kTagLayout.getTagPose(target.getFiducialId()).get();
