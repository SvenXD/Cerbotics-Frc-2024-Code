...
public class FieldCentricDrive extends Command {
  ...
  public FieldCentricDrive(
    ...
    this.xLimiter = new SlewRateLimiter(DriveConstants.MaxLinearSpeed);
    this.yLimiter = new SlewRateLimiter(DriveConstants.MaxLinearSpeed);
  ...
  public void execute() {
    ...
    xSpeed = xLimiter.calculate(xSpeed) * DriveConstants.MaxLinearSpeed;
    ySpeed = yLimiter.calculate(ySpeed) * DriveConstants.MaxLinearSpeed;
    turningSpeed = turningLimiter.calculate(turningSpeed) * DriveConstants.MaxAngularRate;

    m_drive.setControl(
        drive.withVelocityX(xSpeed).withVelocityY(ySpeed).withRotationalRate(turningSpeed));
  }
