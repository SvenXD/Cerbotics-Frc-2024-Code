...
public class Module {
  ...
  private SimpleMotorFeedforward ff =
      new SimpleMotorFeedforward(mkS, mkV, 0.0);
  ...
  public void runSetpoint(SwerveModuleState setpoint, SwerveModuleState torqueFF) {
    ...
    io.runDriveVelocitySetpoint(
        setpoint.speedMetersPerSecond / 1,
        ff.calculate(setpoint.speedMetersPerSecond / 1)
            + ((wheelTorqueNm / 1) * 1));
  ...
  public double getPositionMeters() {
    return inputs.drivePositionRads * 1;
  }

  /** Get velocity of wheel in m/s. */
  public double getVelocityMetersPerSec() {
    return inputs.driveVelocityRadsPerSec * 1;
  }
